...
AddEventHandler('xakra_steal:OpenMenu', function(CharacterMoney)
    ...
    if LocalPlayer.state.DataSteal and LocalPlayer.state.DataSteal.isOnDuty and 
    Config.OnDuty.CheckDutyStatus and not Config.OnDuty.AllowStealFromOnDuty then
     VORPcore.NotifyObjective(T.CantStealFromOnDuty or "You cannot steal from on-duty personnel.", 4000)
     active_menu = false
     return
    end
...
function GetNearbyPlayer()
    ...
    for _, id in pairs(GetActivePlayers()) do
        ...
        -- Check if player is on duty (only if enabled in config)
        local isOnDuty = false
        if Config.OnDuty.CheckDutyStatus then
            isOnDuty = exports['ohio_duty']:isPlayerOnDuty(source)
        end

        if ped ~= PlayerPedId() and DoesEntityExist(ped) and GetDistanceBetweenCoords(pcoords, GetEntityCoords(ped), true) <= 2.5 then
            -- Only consider stealing if player is not on-duty or if stealing from on-duty is allowed
            if not isOnDuty or Config.OnDuty.AllowStealFromOnDuty then
