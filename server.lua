...
AddEventHandler('syn_search:TakeFromsteal', function(obj)
    ...
    -- Check if duty status checking is enabled
    local isOnDuty = false
    if Config.OnDuty.CheckDutyStatus then
        isOnDuty = exports['ohio_duty']:isPlayerOnDuty(steal_source)
    end
    ...
    -- Check on-duty restricted items
    if isOnDuty then
        for _, item in pairs(Config.OnDuty.RestrictedItems) do
            if item == decode_obj.item.name then
                inblacklist = true
                VORPcore.NotifyObjective(_source, T.OnDutyItemRestricted or T.ItemInBlackList, 4000)
            end
        end
    end
